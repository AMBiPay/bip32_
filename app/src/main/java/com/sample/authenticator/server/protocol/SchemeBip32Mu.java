...
public class SchemeBip32Mu implements FidoProtocolScheme {
    private static final String TAG = "SchemeBip32Mu";

    private final TokenMasterKey tokenMasterKey;

    public SchemeBip32Mu() {
    ...
    public String getSchemeName() {
        return "Scheme 2: BIP32-MU Deterministic Key Derivation Scheme";
    }
    ...
    public boolean verifyAuthentication(byte[] state, byte[] credentialId, byte[] clientResponse) {
        try {
            ...
            if (isValid) {
                byte[] rsNew = CustomCryptoUtils.generateRandomBytes(32);
                ServerStorage.saveRcs(credentialId, pk, rsNew);
                Log.d(TAG, "Authentication verification successful, updated rs for next authentication");
            } else {
    ...
    public void updateClientStorage(byte[] credentialId, byte[] newData) {
        ...
        Log.d(TAG, "BIP32-MU scheme does not require client storage");
    }
    ...
    public boolean checkCredential(byte[] serverId, byte[] credential, byte[] revocationKey) {
        ...
        try {
            ...
            boolean matches = Arrays.equals(derivedPkBytes, credential);
